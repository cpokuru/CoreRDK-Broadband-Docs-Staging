...
JS = textwrap.dedent(
    ...
    // ===== Data Loading =====
    // Data is fetched as pre-generated JSON (see rdk-b-components.schema.json /
    // xlsx_to_json.py) instead of being parsed from .xlsx in the browser. To ship
    // a new year's data: edit the workbook, run `python3 xlsx_to_json.py` to
    // regenerate the .json, and drop it next to this HTML file — no HTML rebuild
    // required. If the filename changes, either update DATA_FILE below, or use the
    // "Load Data" button / drag-and-drop panel to pick a new .json file at runtime.
    const DATA_FILE = 'RDK-B_Component_List_2026.json';
    ...
    function sheetsFromData(data) {
      if (!data || !Array.isArray(data.sheets)) {
        throw new Error('Invalid data file: expected an object with a "sheets" array (see rdk-b-components.schema.json)');
      }
      return data.sheets.map(sheet => {
        const headers = (sheet.headers || []).map(normalizeCell);
    ...
    function loadData() {
      loadDataFromUrl(DATA_FILE)
        .then(sheets => initWithSheets(sheets, DATA_FILE))
        .catch(err => {
          console.warn('Auto-load of "' + DATA_FILE + '" failed, falling back to manual picker:', err);
          showWorkbookLoadPanel('Could not auto-load "' + DATA_FILE + '". Drop the .json file here, or click to browse.', true);
        });
    }
    ...
    function bindWorkbookLoader() {
      ...
      if (navLoadBtn) navLoadBtn.onclick = () => { showWorkbookLoadPanel('Drop a .json file here, or click to browse.', false); };
    ...
    function handleWorkbookFile(file) {
      loadDataFromFile(file)
        ...
        .catch(err => {
          ...
          showWorkbookLoadPanel('Could not read "' + file.name + '" — make sure it\'s a valid .json file matching rdk-b-components.schema.json.', true);
        });
    }
    ...
    return textwrap.dedent(
        ...
        <body>
          ...
          <nav id="nav" role="navigation" aria-label="Main navigation">
            ...
            <button class="nav-btn" id="load-workbook-btn" aria-label="Load a different data file"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg> <span>Load Data</span></button>
            <input type="file" id="workbook-file-input" accept=".json" style="display:none">
            ...
            <main id="main" role="main">
              ...
              <div id="content-area">
                <div id="workbook-load-panel">
                  <div id="workbook-drop-zone" tabindex="0" role="button" aria-label="Choose or drop a JSON data file">
                    <div class="wlp-icon">📊</div>
                    <div class="wlp-title">Load the RDK-B component workbook</div>
                    <div class="wlp-sub" id="workbook-load-msg">Drop a .json file here, or click to browse. Next year, just swap in the new file — no rebuild needed.</div>
...
DATA_NAME = "RDK-B_Component_List_2026.json"


def _find_workbook() -> Path | None:
    for candidate in (DOCS_DIR / WORKBOOK_NAME, ROOT / WORKBOOK_NAME):
        if candidate.exists():
            return candidate
    return None
...
def sync_data() -> None:
    """Regenerate docs/RDK-B_Component_List_2026.json from the .xlsx source of truth.

    The .xlsx stays the file people edit day-to-day; this JSON is what
    index.html actually fetches at runtime (see xlsx_to_json.py for the
    conversion logic and rdk-b-components.schema.json for the shape).
    """
    from xlsx_to_json import convert  # local import: keeps `python3 gen_html.py` working standalone

    source = _find_workbook()
    if not source:
        print(f"WARNING: no {WORKBOOK_NAME} found in {DOCS_DIR} or {ROOT}. "
              f"The page's 'Load Data' picker will still work, but auto-fetch will fail "
              f"until a matching .json is placed in {DOCS_DIR}.")
        return

    dest = DOCS_DIR / DATA_NAME
    data = convert(source)
    dest.write_text(__import__("json").dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    n_rows = sum(len(s["rows"]) for s in data["sheets"])
    print(f"Wrote {dest} from {source} ({len(data['sheets'])} sheets, {n_rows} rows total)")

    # Keep the source workbook alongside it in docs/ too, for provenance/editing —
    # GitHub Pages only serves what's inside docs/.
    workbook_dest = DOCS_DIR / WORKBOOK_NAME
    if source != workbook_dest:
        shutil.copy2(source, workbook_dest)
